...
export default function ReviewScreen({ prefill, onSave, onCancel }: Props) {
  ...
  function save() {
    ...
    if (category === 'bills_receipts') {
      ...
      onSave({ ...base, category, totalCents: toCents(totalNum), taxCents: tax.trim() === '' || !Number.isFinite(taxNum) ? null : toCents(taxNum), dateISO: receiptDate });
      return;
    }
    if (category === 'warranty') {
      ...
      onSave({
        ...base, vendor: retailer.trim() || null, category, totalCents: null, taxCents: null, dateISO: null,
    ...
    if (category === 'loyalty') {
      ...
      onSave({
        ...base, category, totalCents: null, taxCents: null, dateISO: null, label: vendor.trim(),
        details: { programType, balanceValue: balance.trim() && Number.isFinite(bal) ? bal : null, expiryDate: expiryDate || null },
      });
      return;
    }
    onSave({ ...base, category, totalCents: null, taxCents: null, dateISO: null, label: vendor.trim() || null });
  }

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scroll}>
      {prefill.imageDataUrl ? <Image source={{ uri: prefill.imageDataUrl }} style={styles.image} /> : null}
      ...
      <Card>
        ...
        {category === 'other' && <Field label="Name / note" value={vendor} onChangeText={setVendor} placeholder="What is this?" />}
      </Card>
...
const styles = StyleSheet.create({
  ...
  primary: { backgroundColor: COLORS.primary, borderRadius: 12, paddingVertical: 15, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, marginTop: 14 },
